...
export async function GET(
  ...
  try {
    ...
    // Strict isolation: only the owner can view
    if (topic.createdByUserId && topic.createdByUserId !== session.user.id) {
      return Response.json({ error: 'Topic not found' }, { status: 404 });
    }
...
export async function PUT(
  ...
  try {
    ...
    if (
      existingTopic.createdByUserId &&
      existingTopic.createdByUserId !== session.user.id
    ) {
      return Response.json(
